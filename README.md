...
> 实验快照：2026-07-24，4 × NVIDIA GeForce RTX 4090 24 GB。以下只报告服务器上已有、
> 可追溯到原始 JSONL/TensorBoard/checkpoint 的结果。`single` 与 `multi5_permute` 的
> 正式预训练、全训练语料 progressive-cloze、22-head P/Q formal validation，以及两个
> `multi5_permute` Q-whole inference-only 诊断均已完成。
...
#### 2.3 正式 P/Q probe 与 Q-whole 诊断

两个 formal pipelines 均完成 22 个 probe heads 和 22 个 checkpoint-reloaded
person-held-out validations。排除固定第一字段 birth date 后，P0 first-token macro 从
`single` 的 6.76% 提升到 `multi5_permute` 的 98.63%；Q-first 六属性 macro 从 12.83%
提升到 98.79%。这是 Allen-Zhu first-token augmentation mechanism 的强定性复刻。

Whole-attribute 端点没有达到论文水平：Q-whole 五属性 macro 从 3.18% 提升到 33.15%，
仍低于 Allen-Zhu bioS multi5+permute 的 92.58%。在不重训原 Q-whole 头的情况下追加
ground-truth `t1` 后，micro accuracy 变为 32.08%（−1.06pp），说明 whole 差距不能简化
为“缺少首 token key”。bad-case MoE route 的受控 difference-in-differences 为 +0.205，
支持 token-conditioned route branching，但不证明 expert 是事实存储单元。

- [SynBioS 实验规范入口](reports/synbios_moe/README.md)
- [single vs multi5+permute formal 报告](reports/synbios_moe/probes/formal_comparison.md)
- [两个 Q-whole val 与全量 whole 对比](reports/synbios_moe/probes/diagnostics/README.md)
...
| 正式 P/Q probe 与 person-held-out validation | 已完成 | first-token 强复刻；whole 仅部分复刻 |
| 两个 Q-whole inference-only diagnostics | 已完成 | 原头不能由 `t1` 直接解锁；bad cases 存在 route branching |
| `single` vs. `multi5_permute` formal 对照 | 已完成 | augmentation 显著改变 first-token 可读性 |
| Matched `single` Q-whole route diagnostic | 尚未运行 | 暂不能把 route contrast 归因于 augmentation |

因此当前结论是：两种 backbone 都充分记忆训练语料；`multi5_permute` 将 first-token
知识从固定文本位置提升为几乎全位置和 name-only 可读，复刻了论文的主要定性机制；
whole-attribute linear readability 虽提升但远未复刻论文数值。Probe validation 的人物只
对 probe-head training held out，仍被 backbone 预训练见过，不能表述为“对新人物泛化”。
MoE route 结果是一个待 matched `single` 对照的机制假设，不是因果存储结论。
...
- SynBioS 项目级入口：[`reports/synbios_moe/README.md`](reports/synbios_moe/README.md)
- Formal probe 与诊断报告：[`reports/synbios_moe/probes/README.md`](reports/synbios_moe/probes/README.md)
