...
> 可追溯到原始 JSONL/TensorBoard/checkpoint 的结果。`single` 已完成正式训练；
> `multi5_permute` 已于 2026-07-21 启动正式训练但尚无最终结果；正式 P/Q probe 尚未运行，
> 不能把运行中指标或结构 smoke test 当作论文结果。

### 摘要

本实验在 100,000 个合成人物的单篇 biography 语料上训练一个 293.49M 总参数、约 124M
token-active 参数的 top-2 MoE Transformer。模型使用 4 卡 FSDP、BF16、global batch 448
训练 540 epochs（17,280 optimizer steps，3.964B scheduled tokens）。稳定配置的总 loss
从 10.9464 降至 0.1932，最小记录值为 0.1921。随后在全部 100,000 篇**训练语料原文**上
执行 progressive cloze：删除每篇文本中的六个真实属性 span，并按原文顺序 greedy 填回，
较早的模型预测成为较晚字段的上下文。600,000 个字段全部达到严格、区分大小写的完全匹配，
100,000 篇 biography 均为 6/6 正确。该结果证明模型对训练分布的精确记忆与优化收敛，
不证明对未见人物、未见属性组合或未见模板的泛化。

### 1. 方法

#### 1.1 数据

正式条件为 SynBioS `single`：seed 1337 生成 100,000 个合成人物，每人一篇 biography，
共 100,000 篇、7,405,102 个 tokenizer tokens。每篇文本包含 `birth_date`、`birth_city`、
`university`、`major`、`company`、`company_city` 六个带精确字符 span 的属性。这里的“原始
bio”指训练时实际使用的生成语料原文，不是现实人物数据。
...
保存在服务器 `/data/mini-train-sys/artifacts/synbios_moe/single/`，Git 中保存其
[manifest 与 SHA256](results/datasets/synbios_moe/single/manifest.json)，不复制 127 MiB
数据 payload。
...
| 训练长度 | 540 epochs；17,280 steps；3,963,617,280 scheduled tokens |
| 数据顺序 | randomized-document packing；shuffle window 1,024；每 rank 4 workers |
| Checkpoint | 每 10 epochs 原子保存 DCP+Adam；保留 epoch 500 safety、530、540 final |
| 正式运行配置 | [`single_fsdp_4gpu.yaml`](configs/synbios_moe/runs/single_fsdp_4gpu.yaml) |
...
完整 100k 验证切为四个连续、不重叠的 25k 分片；聚合器在汇总前拒绝 overlap/gap。
详细协议、两个逐步填空实例和模糊匹配的高估风险见
[`reports/synbios_single_cloze_100k.md`](reports/synbios_single_cloze_100k.md)。
...
`COMMITTED`、runtime、RNG 和配置等小型恢复证据。
...
| `multi5_permute` 正式训练与比较 | **训练中**：4-GPU FSDP，108 epochs / 17,388 steps | 最终 checkpoint 前暂无 single vs. augmentation 对照结论 |

因此，当前最强结论是“训练有效并达到训练集原文的完全 recall”，不是“对新人物达到
100%”。后续论文对比必须完成正在运行的 `multi5_permute` 训练、person-level held-out P/Q validation
和 router analysis，才能回答知识增强是否改变内部知识组织。
...
它们的位置、manifest/hash 和 `COMMITTED` 证据保留在仓库中。
