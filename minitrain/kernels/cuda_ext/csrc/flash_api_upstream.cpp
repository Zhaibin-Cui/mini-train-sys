...
std::vector<at::Tensor> flash_attn_bwd(
    ...
    CHECK_LAST_CONTIGUOUS(dout_input);
    ...
    TORCH_CHECK(dropout_p_value >= 0.0 && dropout_p_value < 1.0, "dropout_p must be in [0, 1)");
    TORCH_CHECK(dropout_p_value == 0.0 || rng_state.numel() == 2, "dropout backward requires forward RNG state");
    at::cuda::CUDAGuard device_guard(q.device());

    // The kernel requires contiguous head_dim but accepts arbitrary outer
    // strides. Making dout contiguous only when necessary avoids a common
    // autograd gradient-layout failure without changing QKV layout.
