...
from experiments.synbios_moe.probes import AttributeProbe, GPT2Codec
...
def audit(args: argparse.Namespace) -> dict[str, object]:
    ...
    for kind in ("p", "q"):
        for attribute in WHOLE_ATTRIBUTES:
            ...
            for split_name, first_data, whole_data, rebuilt_data in (
                ...
                positions = range(6 if kind == "p" else 1)
                for source_index in indices:
                    ...
                    if (
                        ...
                        raise ValueError(
                            f"{kind}/{attribute}/{split_name} source inputs are not aligned"
                        )
                    ...
                    for position in positions:
                        ...
                        if (
                            ...
                            raise ValueError(
                                f"{kind}/{attribute}/{split_name} rebuilt item mismatch"
                            )
                        if actual.input_ids[actual.positions[0]] != (
                            token_id if kind == "p" else codec.eos
                        ):
                            raise ValueError(
                                f"{kind}/{attribute}/{split_name} readout token mismatch"
                            )
                        if kind == "q" and actual.input_ids[-2:] != [token_id, codec.eos]:
                            raise ValueError("Q input is not [..., true_t1, EOS]")
                        checked += 1
            ...
            if tuple(trainable) != expected_prefixes:
                raise ValueError(
                    f"{kind}/{attribute} trainable parameters mismatch: {trainable}"
                )
            ...
            tasks.append(
                {
                    ...
                    "input_protocol": (
                        "P prefix + true_t1, read true_t1"
                        if kind == "p"
                        else "[EOS, name, true_t1, EOS], read final EOS"
                    ),
                    "passed": True,
                }
            )
    result = {
        ...
        "quality_gate_passed": len(tasks) == 10 and all(task["passed"] for task in tasks),
