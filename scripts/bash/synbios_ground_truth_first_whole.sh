...
OUTPUT="${OUTPUT:-$FORMAL/diagnostics/ground_truth_first_whole_rank_matched}"
...
# Tune P and Q independently. The accepted final run uses the prior pilot update budget;
# ranks, full task matrix, and complete validation remain formal-quality.
STEPS="${STEPS:-4000}"
...
Q_EVAL_BATCH_SIZE="${Q_EVAL_BATCH_SIZE:-${Q_VALIDATION_BATCH_SIZE:-3072}}"
...
# short Q tasks do not wait for the longest P task at a fixed wave boundary.
pids=()
...
for index in "${!tasks[@]}"; do
  ...
  if [[ -f "$output" && -f "$probe_output" ]]; then
    echo "skip completed task: $kind/$attribute"
    continue
  fi
