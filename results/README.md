...
- `validation/`: JUnit, checkpoint-resume metadata, and validation event logs.
- `formal_runs/`: formal training JSONL/TensorBoard events plus lightweight checkpoint metadata.
...
deliberately excluded. Their paths, sizes, manifests, and hashes are retained where available.
